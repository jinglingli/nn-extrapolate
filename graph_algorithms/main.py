...
model_types = {'GNN': GNN, 'GNN_E': GNN_E}
...
def test(epoch, dataset, args, model):
    ...
    if epoch%10 == 0:
        print('************ Best model\'s test accuracy: {:.2f}%, test loss: {:.7f}, mape: {:.7f} (best model is from epoch {}) ************\n'.format(best_model_test_acc, best_model_test_loss, best_model_mape_loss, best_epoch))
        logging.info('************ Best model\'s test accuracy: {:.2f}%, test loss: {:.7f}, mape: {:.7f} (best model is from epoch {}) ************\n'.format(best_model_test_acc, best_model_test_loss, best_model_mape_loss, best_epoch))
...
def main():
    ...
    parser.add_argument('--model', type=str, choices=['GNN_E', 'GNN'], default='GNN_E', help='GNN does not use edge information whereas GNN_E uses edge information')
    parser.add_argument('--activation', type=str, choices=['relu', 'tanh', 'linear', 'sigmoid'], default='relu', help='activation function')
    parser.add_argument('--n_iter', type=int, help='number of GNN iterations/layers')
    parser.add_argument('--hidden_dim', type=int, default=256, help='width of MLPs')
    parser.add_argument('--mlp_layer', type=int, default=2, help='number of layers for MLPs in GNN before the last outoupt MLP')
    parser.add_argument('--fc_output_layer', type=int, default=1, help='number of layers for the output(softmax) MLP in GNN')
    parser.add_argument('--graph_pooling_type', type=str, choices=["sum", "mean", "max", "min"],
                            help='Graph-level pooling over all nodes in the graph')
    parser.add_argument('--neighbor_pooling_type', type=str, choices=["sum", "mean", "max", "min"],
                            help='Pooling for over neighboring nodes')
    ...
    parser.add_argument('--loss_fn', type=str, choices=['cls', 'reg', 'mape'], default='reg', help='classification or regression')
    ...
    parser.add_argument('--files_dir', type=str, default='', help='directory to store trained logfiles')
    
    # Data settings
    parser.add_argument('--data', type=str, default='', help='path to the data')
    parser.add_argument('--weight', type=str, default=None, help='indicating whether the input data has edge weights or not')
    parser.add_argument('--edge_feature_size', type=int, default=1, help='size of edge features')
    parser.add_argument('--node_feature_size', type=int, default=3, help='size of node features')
    ...
    with open("./data/%s" %args.data, 'rb') as f:
    ...
    print('************ Best model\'s test accuracy: {:.2f}%, test loss: {:.7f} throughout training (best model is from epoch {}) ************\n'.format(best_model_test_acc, best_model_test_loss, best_epoch))
    logging.info('************ Best model\'s test accuracy: {:.2f}%, test loss: {:.7f} throughout training (best model is from epoch {}) ************\n'.format(best_model_test_acc, best_model_test_loss, best_epoch))
